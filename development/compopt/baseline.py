...
def d_calcNorm(d_vectors, d_out):
	...
	if i<d_vectors.shape[2] and j<d_vectors.shape[1]:
		sm = 0
...
def d_calcVector(d_coords, d_conn, d_out):
	...
	if i<d_coords.shape[1] and j<d_conn.shape[0]:
		jind = d_conn[j, i]
		for k in range(d_coords.shape[0]):
			d_out[k, j, i] = d_coords[k, jind] - d_coords[k, i]

@cuda.jit
def d_calcElongation(d_eta, d_nu, d_x, d_out):
	i, j = cuda.grid(2)

	if i<d_eta.shape[2] and j<d_eta.shape[1]:
		sm = 0
		for k in range(d_eta.shape[0]):
			sm += (d_eta[k,j,i] + d_nu[k,j,i])**2
		d_out[j,i] = sm**0.5 - d_x[j, i]
...
@cuda.jit
def d_calcDilation(d_PD_e, d_m, d_out):
	i = cuda.grid(1)

	if i<d_PD_e.shape[0]:
		if form == 0:
			d_out[i] = 3 / d_m[i] * d_PD_e[i]
		elif form == 1:
			d_out[i] = 2*(2*v - 1)/(v - 1) * d_PD_e[i] / d_m[i]
		else:
			d_out[i] = 2 / d_m[i] * d_PD_e[i]

@cuda.jit
def d_calcDevStrain(d_e, d_dil, d_x, d_out):
	i,j = cuda.grid(2)

	if i < d_e.shape[1] and j < d_e.shape[0]:
		d_out[j, i] = d_e[j, i] - d_dil[i] * d_x[j, i] / 3

@cuda.jit
def d_calcForceState(d_m, d_dil, d_x, d_ed, d_wedx, d_t):
	i, j = cuda.grid(2)

	if i < d_x.shape[1] and j < d_x.shape[0]:
		mi = d_m[i]
		xi = d_x[j, i]
		if xi > 0:
			w = 1/xi
...
for tt in range(1000):
	# if tt%500==0:
	# 	print(tt)
	d_calcVector[((NN + TPB)//TPB, (NB + TPB)//TPB),(TPB, TPB)](d_u, d_conn, d_nu)
	d_calcElongation[((NN + TPB)//TPB, (NB + TPB)//TPB),(TPB, TPB)](d_eta, d_nu, d_x, d_e)
	d_calcPeriDot[(NN + TPB)//TPB, TPB](d_e, d_PD_e)
	d_calcDilation[(NN + TPB)//TPB, TPB](d_PD_e, d_m, d_dil)
	d_calcDevStrain[((NN + TPB)//TPB, (NB + TPB)//TPB),(TPB, TPB)](d_e, d_dil, d_x, d_ed)
	d_calcPeriDot[(NN + TPB)//TPB, TPB](d_ed, d_wedx)
	d_calcForceState[((NN + TPB)//TPB, (NB + TPB)//TPB),(TPB, TPB)](d_m, d_dil, d_x, d_ed, d_wedx, d_t)
