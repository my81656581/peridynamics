...
@cuda.jit
def d_calcElongation(d_u, d_conn, d_eta, d_x, d_nu, d_e):
    i = cuda.grid(1)
    ...
    if i<d_eta.shape[2]:
        ...
        for j in range(d_eta.shape[1]):
            ...
            nu_x = d_coords[0, jind] - ix
            nu_y = d_coords[1, jind] - iy

            d_nu[:,j,i] = (nu_x, nu_y)
            d_e[j,i] = ((d_eta[0,j,i] + nu_x)**2 + (d_eta[1,j,i] + nu_y)**2)**0.5 - d_x[j,i]
...
for tt in range(1000):
	# if tt%500==0:
	# 	print(tt)
	# d_calcVector[(NN + TPB)//TPB,TPB](d_u, d_conn, d_nu)
	d_calcElongation[(NN + TPB)//TPB,TPB](d_u, d_conn, d_eta, d_x, d_nu, d_e)
	d_calcPeriDot[(NN + TPB)//TPB, TPB](d_e, d_PD_e)
	d_calcDilation[(NN + TPB)//TPB, TPB](d_PD_e, d_m, d_dil)
	d_calcDevStrain[(NN + TPB)//TPB,TPB](d_e, d_dil, d_x, d_ed)
